...
RegisterNetEvent("hospital:client:sendMessageCivilianDown",function(coords)
    local street1, street2 = GetStreetNameAtCoord(coords.x, coords.y, coords.z)
    local street1name = GetStreetNameFromHashKey(street1)
    local street2name = GetStreetNameFromHashKey(street2)
    TriggerServerEvent('qb-phone:server:sendNewMail', {
        sender = Lang:t('mail.civilianDownSender'),  
        subject = Lang:t('mail.civilianDownSubject'),
        message = Lang:t('mail.civilianDownMessage') ..street1name.. ' ' .. street2name .. ', '.. Lang:t('mail.civilianDownMessage2') ,
        button = {
            enabled = true,
            buttonEvent = "hospital:client:ambulanceAlert",
            buttonData = coords
        }
    })
end)